...
<div>
    ...
    <?php foreach ($faculty as $item): ?>
        ...
        <table class="table">
            ...
            <?php foreach ($item->faculty->getCg()->contractOnly()->edulevel($eduLevel)->foreignerStatus(false)
                               ->tpgu(false)->all() as $cg): ?>
            <tr>
                <th><?=$cg->specialty->codeWithName?> <?=$cg->getSpecialisationName()?></th>
                <th><?= $cg->formEdu ?></th>
                <th><?= \frontend\widgets\competitive\ButtonWidget::widget(['cgContract'=> $cg]) ?></th>
            </tr>
            <?php endforeach; ?>
        </table>
    <?php endforeach; ?>
