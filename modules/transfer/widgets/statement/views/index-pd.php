        ...
        <table class="table table-bordered">
            <tr class="<?= BlockRedGreenHelper::colorTableBg($statement->countFiles(), $statement->count_pages) ?>">
                <td><?= Html::a('Скачать заявление', ['statement-personal-data/pdf', 'id' =>  $statement->id],
                        ['class' => 'btn btn-warning'])?> <?= FileWidget::widget(['record_id' => $statement->id,
                        'model' => \modules\transfer\models\StatementConsentPersonalData::class ]) ?>
                </td>
            </tr>
            <tr>
                <td>
                    <?= FileListWidget::widget(['record_id' => $statement->id,
                        'model' => \modules\transfer\models\StatementConsentPersonalData::class,  'userId' => $statement->user_id ]) ?>
                </td>
            </tr>
        </table>
    </div>
