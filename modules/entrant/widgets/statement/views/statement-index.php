    ...
    <table class="table table-bordered">
        ...
        <?php if($statement->isStatusAccepted()) : ?>
            <tr>
                <td><?= $statement->statementRejection ?
                        Html::a("Удалить отзыв", ['abiturient/statement/rejection-remove', 'id' =>  $statement->statementRejection->id], ['class'=> 'btn btn-danger', 'data' =>[
                            'confirm'  => "Вы уверены, что хотите отозвать заявление?",
                            'method'=> 'post']]) :
                        Html::a("Отозввть", ['abiturient/statement/rejection', 'id' =>  $statement->id], ['class'=> 'btn btn-info', 'data' =>[
                        'confirm'  => "Вы уверены, что хотите отозвать заявление?",
                        'method'=> 'post']]) ?>
                </td>
            </tr>
        <?php endif; ?>
            <?php if($statement->statementRejection) : ?>
                <tr>
                    <td>
                    <td><?= Html::a('Скачать заявление', ['abiturient/statement-rejection/pdf', 'id' =>  $statement->statementRejection->id],
                            ['class' => 'btn btn-large btn-warning'])?> <?= FileWidget::widget([ 'url' => "abiturient/file/upload",
             ...
             <table class="table" style="background-color: transparent">
                 ...
                 <tr>
                    <td><?= $statementCg->cg->fullName ?></td>
                     ...
                     <td>
                         <?= Html::a('Скачать заявление', ['abiturient/statement-rejection/pdf-cg', 'id' =>  $statementCg->id],
                         ['class' => 'btn btn-large btn-warning'])?> <?= FileWidget::widget([ 'record_id' => $statementCg->id,
                             'model' => \modules\entrant\models\StatementCg::class,  'url' => "abiturient/file/upload", ]) ?>
                     </td>
                         <td colspan="2">
