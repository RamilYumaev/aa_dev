                ...
                'afterRow' => function (Task $model, $index, $grid) {
                    return '<tr><td colspan="5">'.($model->isStatusDone() ? Html::a('Доработка', ['task/rework', 'id' => $model->id, 'status' =>
                            ...
                            Html::a('Не принято', ['task/status', 'id' => $model->id, 'status' =>
                                Task::STATUS_NOT_EXECUTED],['class'=> "btn btn-danger", 'data-confirm'=> "Вы уверены, что хотите изменить статус?"]) : "").
                        ($model->isStatusAcceptedToTime() ? Html::a('Возобновить', ['task/status', 'id' => $model->id, 'status' =>
                            Task::STATUS_RESUMED],['class'=> "btn btn-success", 'data-confirm'=> "Вы уверены, что хотите изменить статус?"]) : "").'</td>
